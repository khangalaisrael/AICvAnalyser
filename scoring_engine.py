from config import SCORE_WEIGHTS, KNOCKOUT_THRESHOLD, VERDICT_HIRE, VERDICT_MAYBE
from role_profiles import RoleProfile


def apply_knockout_filter(ai_result: dict, role_profile: RoleProfile) -> dict | None:
    """Return a rejection dict if candidate fails the knockout filter, else None."""
    missing_must_haves = [
        skill for skill in role_profile["must_have"]
        if skill not in (ai_result.get("matched_skills") or [])
    ]
    if len(missing_must_haves) >= KNOCKOUT_THRESHOLD:
        return {
            "verdict": "REJECT",
            "knockout": True,
            "knockout_reason": f"Missing {len(missing_must_haves)} required skills: {', '.join(missing_must_haves)}",
            "final_score": 0,
...
def _score_years_experience(years: int, min_required: int) -> float:
    """Return 0.0–1.0 based on how candidate's years compare to the minimum."""
    if years <= 0:
        return 0.0
...
def calculate_score(ai_result: dict, role_profile: RoleProfile) -> dict:
    """Compute the weighted scorecard and return the full result dict."""
...
def run_scoring(ai_result: dict, role_profile: RoleProfile) -> dict:
    """Entry point: run knockout filter first, then scorecard if it passes."""
